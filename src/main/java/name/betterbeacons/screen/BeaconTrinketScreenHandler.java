...
public class BeaconTrinketScreenHandler extends ScreenHandler {
    ...
    public double calculatePoints() {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            double val = 0;
            if (item == Items.COPPER_BLOCK) val = Betterbeacons.CONFIG.trinketBeacon.copperValue;
            else if (item == Items.IRON_BLOCK) val = Betterbeacons.CONFIG.trinketBeacon.ironValue;
            else if (item == Items.GOLD_BLOCK) val = Betterbeacons.CONFIG.trinketBeacon.goldValue;
            else if (item == Items.EMERALD_BLOCK) val = Betterbeacons.CONFIG.trinketBeacon.emeraldValue;
            else if (item == Items.DIAMOND_BLOCK) val = Betterbeacons.CONFIG.trinketBeacon.diamondValue;

            basePoints += (val * stack.getCount());

            if (item == Items.NETHERITE_BLOCK) {
                multiplier += (Betterbeacons.CONFIG.trinketBeacon.netheriteMultiplier * stack.getCount());
            }
        }

        // Only count unique blocks for the valuable 6 types
        multiplier += (uniqueBlocks.size() * Betterbeacons.CONFIG.trinketBeacon.uniqueMultiplier);
